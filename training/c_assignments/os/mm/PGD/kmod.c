...
long myIoctl(struct file *fp, unsigned int pid, unsigned long k)
{
	...
	while(i < 768) {
		...
		if((pgd + i) -> pgd) {
			if(i < 768) {
				printk("user space \n");
				phy = (unsigned int)(((pgd + i) -> pgd) >> 12);
				printk("PGD[%d] %lx", i, ((pgd + i) -> pgd));
				pt = (pte_t *)kmap(&mem_map[phy]);
				printk(KERN_CONT"\tvirtual %p\n",pt);
				for(j = 0; j < 1024; j++) {
					if((pt + j) -> pte) {
						printk("PGD[%d]\tPT[%d] %lx\t",i, j, ((((pt + j) -> pte))));
					}
				}
			}
			else {
				...
				for(j = 0; j < 1024; j++) {
					if((pt + j) -> pte) {
						printk("PGD[%d]\tPT[%d] %03lx\t",i, j, ((((pt + j) -> pte))));
