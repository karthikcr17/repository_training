...
long myIoctl(struct file* fp1, unsigned int pid, unsigned long b)
{
	...
	while (vm_area) {
		...
		printk ("%p-%p ", (int *)(vm_area -> vm_start), 
						  (int *) (vm_area -> vm_end));
		
		/* printing permissions */
		printk ("%c%c%c%c", ((vm_area -> vm_flags) & VM_READ)? 'r': '-',
							((vm_area -> vm_flags) & VM_WRITE)? 'w': '-', 
							((vm_area -> vm_flags) & VM_EXEC)? 'x': '-', 
							((vm_area -> vm_flags) & VM_SHARED)? 's': 'p');	//shared or private
		/*offset*/
		...
		}	/* checking for stack and heap regions */
			if (((vm_area -> vm_start) < (vm_area -> vm_mm -> start_stack)) 
				&& ((vm_area -> vm_end) > (vm_area -> vm_mm -> start_stack))) {
				printk ("%s ", "[stack]");
			} else if (vm_area -> vm_start == vm_area -> vm_mm -> start_brk) {
				printk ("%s\n", "[heap]");
			}
			else {
				printk ("\n");
