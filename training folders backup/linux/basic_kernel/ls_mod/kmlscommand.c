...
/* ioctl for lsmod command */

long myIoctl(struct file* fp1, unsigned int pid, unsigned long b)
{
	struct module *mod = NULL;		/* descriptor for current module*/
	struct module_use *temp_moddep = NULL;	/* descriptor for dependecny module*/

	printk ("name            size     used      by");

	list_for_each_entry (mod, &(modules), list) { 
		
		printk(KERN_INFO"%s\t", mod -> name); //printing the name of the module
		
		list_for_each_entry (temp_moddep, &(mod -> source_list), source_list) {
			/* printing name of the dependency modules*/
			printk(KERN_INFO"%s", temp_moddep -> source -> name);
		}
	printk("\n");
	}
