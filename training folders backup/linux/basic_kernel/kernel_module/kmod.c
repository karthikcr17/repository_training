...
long myIoctl(struct file* fp1, unsigned int pid, unsigned long b)
{
	...
	mod = THIS_MODULE; 	//gives the starting address of struct module of current module

	printk(KERN_INFO "my_pid in from task_struct : %d \n", my_task->pid);
	printk(KERN_INFO "my_pid in from application : %d \n", pid);
	printk(KERN_INFO "comm string is : %s \n", my_task -> comm);
	printk(KERN_INFO "open ioctl succesfull");
	printk(KERN_INFO "my module name is :%s\n", mod -> name);
