...
long myIoctl(struct file* fp1, unsigned int pid, unsigned long b)
{
	...
	my_task = current; //current macro returns the starting address of task_struct
	mod = THIS_MODULE; //THIS_MODULE gives the starting address of struct module

	printk(KERN_INFO "mm value: %p\n", my_task -> mm);
	printk(KERN_INFO "mmap value: %p\n", my_task -> mm -> mmap);
	printk(KERN_INFO "files value:%p \n", my_task -> files);
	printk(KERN_INFO "fdt value:%p \n", my_task -> files -> fdt);
	printk(KERN_INFO "f_count  value:%d \n", (my_task -> files -> fdt -> fd[b] ->f_count));
