 ...
 */
nt myRelease(struct inode *in, struct file *fp)
{
...
long myIoctl(struct file* fp1, unsigned int pid, unsigned long b)
{
	...
	my_task = current;	// current macro gives descriptor for the current process
	vm_area = my_task -> mm -> mmap;
	mod = THIS_MODULE; //macro gives the descriptor for the current module
	...
	printk(KERN_INFO "file system pointer is %p\n", my_task ->files);
	printk(KERN_INFO "mm pointer is : %p \n", my_task -> mm );
	return 0;
}
i
module_init(myInit);
