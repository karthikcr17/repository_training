...
long myIoctl(struct file* fp1, unsigned int pid, unsigned long b)
{
	...
	struct task_struct *n_task = NULL;

	task = current;


		list_for_each_entry (n_task, &(task -> tasks), tasks) 
			printk (KERN_INFO"%20s  %5d  %5d  %5d\n", n_task -> comm, n_task -> tgid, n_task -> pid, n_task -> real_parent -> tgid);



	printk (KERN_INFO"name : %s pid : %d tgid: %d ppid : %d\n", task -> comm, task -> tgid, task -> pid, task -> real_parent -> tgid);
	return 0;
}
