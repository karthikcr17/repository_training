...
/*	printk(KERN_INFO "my vm_strat address is code :%p", (int *) (vm_area -> vm_start));
	printk(KERN_INFO "my vm_end address is code :%p\n", (int *) (vm_area -> vm_end));
	printk(KERN_INFO "my vm_end address is data :%p\n", (int *) (vm_area -> vm_next -> vm_start));
	printk(KERN_INFO "my vm_end address is data :%p\n", (int *) (vm_area -> vm_next -> vm_end));
	printk(KERN_INFO "my vm_end address is heap :%p\n", (int *) (vm_area -> vm_next -> vm_next -> vm_start));
	printk(KERN_INFO "my vm_end address is heap :%p\n", (int *) (vm_area -> vm_next -> vm_next -> vm_end));
	printk(KERN_INFO "my vm_end address is stack :%p\n", (int *) (vm_area -> vm_next -> vm_next -> vm_next -> vm_start));
	printk(KERN_INFO "my vm_end address is stack :%p\n", (int *) (vm_area -> vm_next -> vm_next -> vm_next -> vm_end));*/
/*	while (vm_area) {
		printk (KERN_INFO"\nstarting address: %p", (int *)(vm_area -> vm_start));
		printk (KERN_INFO"\nending address: %p", (int *)(vm_area -> vm_end));
		vm_area = vm_area -> vm_next;
	}*/


	printk(KERN_INFO "File %p\n", my_task -> files);
	printk(KERN_INFO "Fdt %p\n", my_task -> files -> fdt -> fd[b] -> f_count);
