...
/* ioctl for verfiying copy on write mechanism */
...
long myIoctl(struct file *fp, unsigned int arg1, unsigned long uv_addr)
{
	...
	unsigned long pgdind;	//index in PGD
	unsigned long ptind;	// index in pt
	unsigned long pgind;	//index for pageframe
	...
	pgdind = ((uv_addr) >> 22);
	ptind = (((uv_addr) << 10) >> 22);
	pgind = (uv_addr) & 0x00000FFF;

	if(pgdptr[pgdind].pgd < 0x38000000) {	//checking for low mem and high mem
	...
	printk("page frame %lx\n\n", pt[ptind]);

	/* unmapping the mapped kernel virtual address */
	kunmap(&mem_map[(pgdptr[pgdind].pgd) >> 12]);
